    ...
    <div class="container w-25 border p-4 my-4">
        <div class="row mx-auto">
            <form action="{{ route('categories.store') }}" method="POST">
                ...
                @enderror
                <div class="mb-3">
                    <label for="name" class="form-label">Nombre de el rol</label>
                    <input type="text" name="name" class="form-control">
                </div>
                <div class="mb-3">
                    <label for="color" class="form-label">Color de el rol</label>
                    <input type="color" name="color" class="form-control">
                </div>
                <button type="submit" value="Crear categoría" class="btn btn-primary">Crear nuevo rol</button>
            </form>
                    ...
                    <div class="modal fade" id="modal{{$category->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            ...
                            <div class="modal-body">
                                Al eliminar el rol <strong>{{ $category->name }}</strong> se eliminan todos los usuarios asignados a este rol. 
                                ¿Estás seguro de que deseas eliminar el rol <strong>{{ $category->name }}</strong>?
                            </div>
                            <div class="modal-footer">
                                ...
                                <form action="{{ route('categories.destroy', ['category' => $category->id]) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="btn btn-danger">Eliminar el rol</button>
                                </form>
